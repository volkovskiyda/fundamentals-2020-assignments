...
class WS02ActorsAdapter : RecyclerView.Adapter<ActorsViewHolder>() {
    ...
    override fun getItemViewType(position: Int): Int =
        if (actors.isEmpty()) R.layout.item_actors_empty else R.layout.item_actors_data
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ActorsViewHolder =
        when (viewType) {
            R.layout.item_actors_empty -> EmptyViewHolder(
                LayoutInflater.from(parent.context)
                    .inflate(R.layout.item_actors_empty, parent, false)
            )
            R.layout.item_actors_data -> DataViewHolder(
                LayoutInflater.from(parent.context)
    ...
    override fun onBindViewHolder(holder: ActorsViewHolder, position: Int) {
        ...
        when (holder) {
            ...
            is DataViewHolder -> holder.somePublicMethod(imageOption, actors[position])
        }
    }
...
private class DataViewHolder(itemView: View) : ActorsViewHolder(itemView) {
    ...
    fun somePublicMethod(options: RequestOptions, actor: Actor) {
        ...
        oscarState.text =
            context.getString(R.string.fragment_actors_avatar_oscar_state_text, actor.hasOscar.toString())
    }
}
